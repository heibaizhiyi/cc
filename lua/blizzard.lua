...
function SetAmbientDaySound(inLabel)
	...
	if bj_dayAmbientSound ~= nil and bj_dayAmbientSound ~= 0 then
...
function SetAmbientNightSound(inLabel)
	...
	-- Stop old sound, if necessary
	if bj_nightAmbientSound ~= nil and bj_nightAmbientSound ~= 0 then
...
function GetInventoryIndexOfItemTypeBJ(whichUnit, itemId)
	...
	for _ in _loop_() do
		...
		if indexItem ~= nil and indexItem ~= 0 and GetItemTypeId(indexItem) == itemId then
			return index + 1
		end
...
function UnitInventoryCount(whichUnit)
	...
	for _ in _loop_() do
		if UnitItemInSlot(whichUnit, index) ~= nil and UnitItemInSlot(whichUnit, index) ~= 0 then
			count = count + 1
		end
...
function ReplaceUnitBJ(whichUnit, newUnitId, unitStateMethod)
	...
	if IsUnitType(oldUnit, UNIT_TYPE_HERO) and IsUnitType(newUnit, UNIT_TYPE_HERO) then
		...
		for _ in _loop_() do
			...
			if indexItem ~= nil and indexItem ~= 0 then
