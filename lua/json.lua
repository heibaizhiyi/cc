...
function encode_map.table(t)
    ...
    if statusVisited[t] then
        ...
        return "'${circular reference " .. tostring(t) .. "}'"
    end
    statusVisited[t] = true
    if type(first_val) == 'string' or first_val ~= 1 then
        local keys = {}
        for k in next, t do
            if type(k) ~= "string" then
                --print_r(t)
                --error("key只能是字符串:" .. type(k) .. " [==] " .. tostring(k) .. tostring(t[k]))
                print("key只能是字符串:" .. type(k) .. " [==] " .. tostring(k))
        ...
        for k in next, t do
            if math_type(k) ~= "integer" or k <= 0 then
                print_r(t)
                error("数组的key只能是数字:" .. type(k) .. " [==] " .. tostring(k) .. tostring(t[k]))
            end
            count = count + 1
            if max < k then
                max = k
